...
import wandb
from conf import IterMaskFirst_config as config
...
def train(conf):
    ...
    for step in tqdm(range(start_step, end_step), position=0):
            ...
            if (
                ...
                with torch.no_grad():
                    ...
                    for samples_h, samples_l in zip(dataloader_val_h, dataloader_val_l):
                        ...
                        # healthy samples
                        samples_h = (samples_h * 6) - 3
                        input_h = create_condition(samples_h)
                        rec_h = model(input_h)
                        ...
                        # lesion samples
                        samples_l = (samples_l * 6) - 3
                        input_l = create_condition(samples_l)
                        rec_l = model(input_l)
        ...
        if (
            ...
            up_images = wandb.Image(upload_images(images[0:conf.training.num_up],mode='L'))
            wandb.log({"Images": up_images})

            up_recon = wandb.Image(upload_images(rec[0:conf.training.num_up].detach(),mode='L'))
