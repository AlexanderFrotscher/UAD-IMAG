...
from conf import RD_eval as config
        ...
        for image, label in pbar:
            ...
            for my_tensor in split:
                ...
                for j in range(len(feats)):
                    my_map = 1 - F.cosine_similarity(feats[j], rec[j])
                    my_map = torch.unsqueeze(my_map, dim=1)
                    my_map = F.interpolate(
                        my_map,
                        my_tensor.shape[-2:],
                        mode="bilinear",
                        align_corners=True,
