...
def pad_nd_image(
    ...
    pad_list = [[0, 0]] * num_axes_nopad + torch.stack(
        (pad_below, pad_above), -1
    ).tolist()
...
def compute_importance_map(
    ...
    if mode == "constant":
        ...
        for i in range(len(patch_size)):
            ...
            importance_map = (
                importance_map.unsqueeze(-1) * x[(None,) * i] if i > 0 else x
            )
    else:
        raise ValueError(
    ...
    importance_map = torch.clamp_(importance_map.to(torch.float), min=min_non_zero).to(
        dtype
    )
...
def fold(image, image_size, patch_size, steps):
    ...
    output = torch.zeros(
        *image.shape[:2], numel_image, device=image.device, dtype=image.dtype
    )
...
def sliding_window_inference(
    ...
    **kwargs,
    ...
    image_size_padded = [
        d + s - (d - p) % s if (p != d) and ((d - p) % s != 0) else d
        for d, s, p in zip(image_size_padded, steps, patch_size)
    ]
    image_padded, slicer = pad_nd_image(
        image=image, new_shape=image_size_padded, return_slicer=True
    )
    ...
    if sw_batch_size is None:
        vec_t = torch.ones(image_padded_unfolded.shape[0], device=sw_device) * timestep
        pred = predictor(image_padded_unfolded.to(sw_device), vec_t)
        ...
        for i in range(0, image_padded_unfolded.shape[0], sw_batch_size):
            curr_batch = image_padded_unfolded[i : i + sw_batch_size].size(0)
            vec_t = torch.ones(curr_batch, device=sw_device) * timestep
            pred = predictor(
                image_padded_unfolded[i : i + sw_batch_size].to(sw_device), vec_t
            )
            if isinstance(pred, torch.Tensor):
    ...
    for k, output_unfolded in outputs_unfolded.items():
        ...
        weights_unfolded = weights_unfolded.expand(
            list(output_unfolded.shape[: -len(patch_size)])
            + list((-1,) * len(patch_size))
        )
