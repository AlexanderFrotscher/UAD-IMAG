...
from loading import CropForeground
...
class FPI(AbstractTransform):
    def __init__(
        ...
        self.normalize_fp = "mean" if normalize_fp else normalize_fp

        assert anomaly_interpolation in ["linear", "poisson", "none"], (
            "Anomaly interpolation method not recognized. Please choose between 'linear' and 'poisson'."
        )
        ...
        if anomaly_interpolation == "poisson":
            try:
                ...
                raise ImportError(
                    "Please install pie-torch: https://pypi.org/project/pie-torch/"
                )

        transforms_fp = [
            CropForeground(key_input=self.image_key),
            RandomCropTransform(crop_size=self.anom_patch_size),
        ]

        if self.fp_augmentations:
            transforms_fp.extend(
                [
                    BrightnessTransform(mu=0.0, sigma=0.1, p_per_sample=0.2),
                    GaussianNoiseTransform(noise_variance=(0.0, 0.2), p_per_sample=0.2),
    ...
    def randomize(self):
        ...
        if do_anomaly:
            ...
            alpha = (
                random.random() * (self.alpha_range[1] - self.alpha_range[0])
                + self.alpha_range[0]
            )
            return do_anomaly, fp, alpha
        else:
            return do_anomaly, None, np.array(0.0)
    ...
    def __call__(self, **data_dict):
        ...
        for n, (image, shape, anom_center) in enumerate(
            zip(
                data_dict[self.image_key],
                data_dict[self.shape_key],
                data_dict["anom_center"],
            )
        ):
            ...
            if do_anomaly:
                ...
                if self.normalize_fp:
                    if mask_non_zeros.any():
                        ...
                        fp_mean, src_mean = (
                            masked_voxels_fp.mean(),
                            masked_voxels_image.mean(),
                        )
                        ...
                        if self.normalize_fp == "mean":
                            ...
                            fp_std, src_std = (
                                masked_voxels_fp.std(),
                                masked_voxels_image.std(),
                            )
                            fp_std = (
                                fp_std if not np.isnan(fp_std) and fp_std > 0 else 1
                            )
                            src_std = (
                                src_std if not np.isnan(src_std) and src_std > 0 else 1
                            )
                            ...
                            fp_min, src_min = (
                                masked_voxels_fp.min(),
                                masked_voxels_image.min(),
                            )
                            fp_max, src_max = (
                                masked_voxels_fp.max(),
                                masked_voxels_image.max(),
                            )

                            fp_big = (fp_big - fp_min) / (fp_max - fp_min + 1e-3)
                            fp_big = (fp_big * (src_max - src_min)) + src_min

                        fp_big = fp_big.clip(0.0, 1.0)
                ...
                if self.anomaly_interpolation == "linear":
                    ...
                    raise NotImplementedError(
                        "Poisson interpolation not implemented yet."
                    )
