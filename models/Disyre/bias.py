...
class BiasCorruption(AbstractTransform):
    ...
    def randomize(self, image, shape):
        ...
        if do_anomaly:
            ...
            if self.quantized_bias_codebook is not None:
                ...
                voxels_per_q = np.matmul(
                    (shape > 0).flatten()[None].astype("int"),
                    image_q.reshape(-1, self.codebook_k),
                )
                voxels_per_q = voxels_per_q.squeeze(0)
                centroids_selected = np.random.rand(self.codebook_k) < (
                    voxels_per_q / (voxels_per_q.sum() + 1e3)
                )
                # If all centroids are False set all to True
                centroids_selected[centroids_selected.sum() == 0] = True
                mask_ranges = (
                    np.matmul(
                        np.expand_dims(image_q, -2),
                        centroids_selected[None, None, None, :, None].astype("int"),
                    )
                    .squeeze(-1)
                    .squeeze(-1)
                )
                ...
                mask_bias = augment_gaussian_blur(
                    mask_ranges.astype(np.float32)[None], sigma_range=(1, 1)
                )[0]
            else:
                mask_bias = 1.0
            ...
            return do_anomaly, 0.0, np.array(0.0), np.zeros_like(shape)
    ...
    def __call__(self, **data_dict):
        ...
        for n, (image, shape) in enumerate(
            zip(data_dict[self.image_key], data_dict[self.shape_key])
        ):
            ...
            if do_anomaly:
                ...
                bias_corrupted = (
                    image * (1 - bias_shape) + (image + alpha * bias_sign) * bias_shape
                )
                bias_corrupted = np.clip(bias_corrupted, 0.0, 1.0)
