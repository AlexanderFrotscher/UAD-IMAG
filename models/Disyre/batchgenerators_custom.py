...
def augment_spatial_2(
    ...
    if seg is not None:
        if dim == 2:
            seg_result = np.zeros(
                (bs, seg[0].shape[0], patch_size[0], patch_size[1]), dtype=np.float32
            )
        else:
            seg_result = np.zeros(
                (bs, seg[0].shape[0], patch_size[0], patch_size[1], patch_size[2]),
                dtype=np.float32,
            )

    if dim == 2:
        data_result = np.zeros(
            (bs, data[0].shape[0], patch_size[0], patch_size[1]), dtype=np.float32
        )
    else:
        data_result = np.zeros(
            (bs, data[0].shape[0], patch_size[0], patch_size[1], patch_size[2]),
            dtype=np.float32,
        )
