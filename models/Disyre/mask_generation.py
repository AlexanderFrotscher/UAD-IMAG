...
from loading import DataLoader3D


class MaskGenerator:
    def __init__(self, patch_size, spatial_prob=0.0, smooth_prob=0.0):
        self.patch_size = patch_size
        self.spatial_dims = len(patch_size)
        self.spatial_prob = spatial_prob
        self.smooth_prob = smooth_prob
        self.scale = (1.0, 1.0)  # This is here to enable resaling masks
    ...
    def random_smooth(self, mask):
        mask = augment_gaussian_blur(mask, sigma_range=(0, 5), p_per_channel=0.7)
        return mask
...
class RandomSquare(MaskGenerator):
    def __init__(
        ...
        if isinstance(min_anom_sizes, (tuple, list)):
            assert len(min_anom_sizes) == self.spatial_dims, (
                "min_anom_sizes must have the same length as spatial_dims or be an int"
            )
        else:
        ...
        max_anom_sizes = (
            max_anom_sizes
            if max_anom_sizes is not None
            else [p - 20 for p in patch_size]
        )

        if isinstance(max_anom_sizes, (tuple, list)):
            assert len(max_anom_sizes) == self.spatial_dims, (
                "max_anom_sizes must have the same length as spatial_dims or be an int"
            )
        else:
    ...
    def generate_mask(self, *args, **kwargs):
        anom_size = [
            random.randint(a, b)
            for a, b in zip(self.min_anom_sizes, self.max_anom_sizes)
        ]
        anom_center = [
            random.randint(a_s // 2, p_s - a_s // 2)
            for a_s, p_s in zip(anom_size, self.patch_size)
        ]
        ...
        if self.spatial_dims == 3:
            out[
                :,
                anom_center[0] - anom_size[0] // 2 : anom_center[0] + anom_size[0] // 2,
                anom_center[1] - anom_size[1] // 2 : anom_center[1] + anom_size[1] // 2,
                anom_center[2] - anom_size[2] // 2 : anom_center[2] + anom_size[2] // 2,
            ] = 1
...
class RandomSphere(MaskGenerator):
    def __init__(
        ...
        self.max_anom_size = (
            max_anom_size if max_anom_size is not None else min(patch_size) - 20
        )
        self.mesh = np.stack(
            np.meshgrid(*[np.linspace(0, i, i) for i in self.patch_size], indexing="ij")
        )

    def generate_mask(self, *args, **kwargs):
        anom_size = random.randint(self.min_anom_size, self.max_anom_size)
        anom_center = [
            random.randint(anom_size // 2 + 10, p_s - anom_size // 2 - 10)
            for p_s in self.patch_size
        ]
...
class RandomMask(MaskGenerator):
    def __init__(
        ...
        self.shape_loader = DataLoader3D(
            dataset,
            batch_size=1,
            infinite=True,
    ...
    def __call__(self, **data_dict):
        ...
        for n in range(bs):
            if self.not_in_background:
                ...
                valid_indices = (
                    (indices_nonzero >= margin)
                    & (indices_nonzero < np.array(image_size)[None] - margin)
                ).all(1)
                if valid_indices.sum() == 0:
                    anom_center.append(
                        np.array(
                            [
                                random.randint(a // 2, i - (a // 2))
                                for i, a in zip(image_size, self.anom_patch_size)
                            ]
                        )
                    )
                else:
                    try:
                        anom_center.append(
                            random.choice(indices_nonzero[valid_indices])
                        )
                    except:
...
class CreateRandomShape(AbstractTransform):
    ...
    def __init__(
        ...
        if self.p_squaremask > 0:
            self.square_mask_gen = RandomSquare(
                anom_patch_size, smooth_prob=smooth_prob, **square_kwargs
            )
        else:
            self.square_mask_gen = None
        if self.p_spheremask > 0:
            self.sphere_mask_gen = RandomSphere(
                anom_patch_size, smooth_prob=smooth_prob, **sphere_kwargs
            )
        else:
            self.sphere_mask_gen = None
        if self.p_randommask > 0:
            self.random_mask_gen = RandomMask(
                anom_patch_size,
                masks_spatial_dims=3,
    ...
    def __call__(self, **data_dict):
        ...
        for i in range(bs):
            mg = random.choices(
                [self.square_mask_gen, self.sphere_mask_gen, self.random_mask_gen],
                weights=[self.p_squaremask, self.p_spheremask, self.p_randommask],
            )

            mask_anom = mg[0]()

            assert "anom_center" in data_dict, (
                "Anomaly center not found in data_dict. Please use GetRandomLocation transform to generate it."
            )
            ...
            if len(self.anom_patch_size) == 3:
                shape_i[
                    :,
                    anom_center[0] - self.anom_patch_size[0] // 2 : anom_center[0]
                    + self.anom_patch_size[0] // 2,
                    anom_center[1] - self.anom_patch_size[1] // 2 : anom_center[1]
                    + self.anom_patch_size[1] // 2,
                    anom_center[2] - self.anom_patch_size[2] // 2 : anom_center[2]
                    + self.anom_patch_size[2] // 2,
                ] = mask_anom
