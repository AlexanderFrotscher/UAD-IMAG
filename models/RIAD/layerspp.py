...
class AttnBlockpp(nn.Module):
  ...
  def __init__(self, channels, rescale=False, init_scale=0.):
    super().__init__()
...
class AttentionBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        h = self.proj_out(h)
        if self.rescale:
          return (x + h).reshape(b, c, *spatial) / np.sqrt(2.)
        else:
...
class CausalSelfAttention(nn.Module):
    def __init__(self, channels: int, embed_dim: int=64, rescale: bool=True, is_causal: bool=True, dropout=0.1):
        super().__init__()
    ...
    def forward(self, x):
        ...
        with sdpa_kernel(SDPBackend.EFFICIENT_ATTENTION):
          a = F.scaled_dot_product_attention(query, key, value, attn_mask=None, is_causal=self.is_causal)
...
class ResnetBlockBigGANpp(nn.Module):
  def __init__(self, act, in_ch, out_ch=None, up=False, down=False,
               dropout=0.1, rescale=True):
    super().__init__()
    ...
    self.out_layers = nn.Sequential(
      nn.GroupNorm(num_groups=min(out_ch // 4, 32), num_channels=out_ch, eps=1e-6),
      nn.Dropout(dropout),
