...
class UNet(nn.Module):
  ...
  def __init__(self, config):
    ...
    self.pre_conv = pre_conv = config.model.pre_conv
    ...
    if pre_conv:
      self.all_resolutions = all_resolutions = [(config.data.image_size//2) // (2 ** i) for i in range(num_resolutions)]
    else:
    ...
    AttnBlock = functools.partial(layerspp.CausalSelfAttention,
    ...
    modules.append(layers.zero_module(conv3x3(in_ch, channels)))
