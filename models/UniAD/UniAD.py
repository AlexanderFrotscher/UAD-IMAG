...
class UniAD(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.num_queries = config.model.feature_size * config.model.feature_size
        self.feature_jitter = config.model.feature_jitter
        self.feature_jitter_scale = config.model.feature_jitter_scale
        self.feature_jitter_prob = config.model.feature_jitter_prob
        self.pos_embed = build_position_embedding(
            config.model.pos_embed_type,
        ...
        self.transformer = Transformer(
            config.model.hidden_dim,
            config.model.feature_size,
            config.model.neighbor_mask,
            config.model.neighbor_size,
            config.model.nhead,
            ...
            config.model.normalize_before,
        )
        self.input_proj = nn.Linear(sum(feature_dimensions), config.model.hidden_dim)
        self.output_proj = nn.Linear(config.model.hidden_dim, sum(feature_dimensions))

        for m in [
            *self.transformer.modules(),
            *self.input_proj.modules(),
            *self.output_proj.modules(),
        ]:
    ...
    def forward(self, input):
        ...
        feature_tokens = features.permute(2, 3, 0, 1).contiguous()
        feature_tokens = feature_tokens.view(-1, b, c)
        ...
        feature_rec_tokens = feature_rec_tokens.permute(1, 2, 0).contiguous()

        feature_rec = feature_rec_tokens.view(b, c, *spatial)
        """
        ...
        pred = torch.sqrt(
            torch.sum((feature_rec - input) ** 2, dim=1, keepdim=True)
        )  # B x 1 x H x W
...
class TransformerDecoderLayer(nn.Module):
    def __init__(
        ...
        super().__init__()
        num_queries = feature_size * feature_size
...
class PositionEmbeddingSine(nn.Module):
    ...
    def forward(self, tensor):
        not_mask = torch.ones((self.feature_size, self.feature_size))  # H x W
