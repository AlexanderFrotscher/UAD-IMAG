...
from fae_models import FeatureReconstructor
from pytorch_ssim import SSIMLoss, my_SSIMLoss
from torch import optim
from torchvision.transforms import v2
from tqdm import tqdm

import wandb
from conf import FAE_config as config
...
def train(conf):
    ...
    model = FeatureReconstructor(conf).to(device)
    ...
    ssim = my_SSIMLoss(window_size=conf.model.window_size)
    ssim_val = SSIMLoss(window_size=11, size_average=False)
    ...
    for step in tqdm(range(start_step, end_step), position=0):
            ...
            if (
                ...
                with torch.no_grad():
                    ...
                    for samples_h, samples_l in zip(dataloader_val_h, dataloader_val_l):
                        ...
                        small_loss = ssim_val(feat_h, rec_h)
                        small_loss = (
                            torch.mean(
                        ...
                        small_loss = ssim_val(feat_l, rec_l)
                        small_loss = (
                            torch.mean(
                                small_loss.reshape(small_loss.shape[0], -1), dim=-1
                            )
                ...
                wandb.log(
                    {"SSIM-Val-H": loss_h.item(), "SSIM-Val-L": loss_l.item()},
                    step=step,
                )
        ...
        if step % conf.training.log_freq == 0 and accelerator.is_main_process:
            ...
            wandb.log({"SSIM": loss.item()}, step = step)
